...
// validates the `Input` string, the `Node`s, and `State`s
pred init {
    ...
    all s: State | {
        // for all States `s` with index less than the length of the Input, there is some next State `next` such that Trace records `s` mapping to `next` and the index of `next` is the index of `s` + 1, or we can skip from `s` to `next` and `next` has the same index as `s`
...
// validates the `Nodes` transition correctly according to the `Input` string, i.e. matches the two objects
